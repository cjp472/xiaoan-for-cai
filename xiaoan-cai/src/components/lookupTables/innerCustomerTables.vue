...
<script>
    ...
    export default {
        ...
        data() {
            return {
                lightImg: "../../static/img/tables/totlo.png",
                XGText: "信公提示",
                XGText2: "数据来源：",
                XGText3: "数据更新于",
                Custom: "用户自定义",
        ...
        methods: {
            ...
            getAnnualReportByYear(index) {
                ...
                this.$http({
                    method: 'GET',
                    url: this.$api.host + "permission/getAnnualReportByYear",
                    params: {
                    ...
                    if (res.data.returnCode == 1) {
                        ...
                        * */
                        if (this.stockMarkets == "SH") {
                            ...
                            this.$toast({message: '未知错误！', position: 'middle', duration: 1000});
                        }
                        /**/
                        ...
                        * */
                        ...
                        this.$messagebox.confirm('是否获取最新年报数据，所有修改的数据将丢失，你确定要继续吗？', '').then((res) => {
                            this.restArr();
                            /**/
                        }).catch((err) => {
                            //console.log(err);
            ...
            react() {
                this.$http({
                    method: 'GET',
                    url: this.$api.host + "permission/resetAnnualReport",
                    params: {}
                }).then((res) => {
                    if (res.data.returnCode == 1) {
            ...
            customerGlobal() {
                this.$http({
                    method: 'GET',
                    url: this.$api.host + "permission/getAnnualReportInside",
                    params: {
                        code: this.$route.query.code,
                    }
                }).then((res) => {
            ...
            keep() {
                if (this.totalAssetsVal.trim().length !== 0 || this.YtotalAssetsVal.trim().length !== 0 || this.netAssetsVal.trim().length !== 0 || this.YnetAssetsVal.trim().length !== 0 || this.netProfitVal.trim().length !== 0 || this.totalIncomeVal.trim().length !== 0 || this.incomeVal.trim().length !== 0 || this.businessIncomeVal.trim().length !== 0 || this.costsVal.trim().length !== 0) {
                    if (this.rmoney(this.totalAssetsVal) !== this.rmoney(this.totalAssetsVals) || this.rmoney(this.YtotalAssetsVal) !== this.rmoney(this.YtotalAssetsVals) || this.rmoney(this.netAssetsVal) !== this.rmoney(this.netAssetsVals) || this.rmoney(this.YnetAssetsVal) !== this.rmoney(this.YnetAssetsVals) || this.rmoney(this.netProfitVal) !== this.rmoney(this.netProfitVals) || this.rmoney(this.totalIncomeVal) !== this.rmoney(this.totalIncomeVals) || this.rmoney(this.incomeVal) !== this.rmoney(this.incomeVals) || this.rmoney(this.businessIncomeVal) !== this.rmoney(this.businessIncomeVals) || this.rmoney(this.costsVal) !== this.rmoney(this.costsVals)) {
                        ...
                        this.$http({
                            method: 'POST',
                            url: this.$api.host + "permission/saveInsideAnnualReport",
                            params: {
                                id: this.IncomeId,
                                year: this.yareDate,
                            ...
                            if (res.data.returnCode = 1) {
                                ...
                                setTimeout(() => {
                                    ...
                                    this.$toast({message: '保存成功！', position: 'middle', duration: 1000});
                                }, 600)
                        ...
                        if (this.totle == true) {
                            ...
                            setTimeout(() => {
                                this.$nextTick(() => {
                                    ...
                                    //this.$toast({message: '编辑后保存！', position: 'middle', duration: 1000});
                    ...
                    /**/
                    // //console.log("this.$toast({message: '编辑后保存！', position: 'middle', duration: 1000});");
                    //this.$toast({message: '编辑后保存！', position: 'middle', duration: 1000});
                    /**/
                }
            },
            ...
            restArr() {
                this.$http({
                    method: 'GET',
                    url: this.$api.host + "permission/resetInsideAnnualReport",
                    params: {
                        code: this.$route.query.code,
                    }
                }).then((res) => {
                    if (res.data.returnCode == 1) {
                        ...
                        this.matteSh = false;
                        this.$toast({message: '暂无本公司年报数据！', position: 'middle', duration: 1000});
                    }
                }).catch((err) => {
                })
            },
            ...
            sustain() {
                if (this.restat == true) {
                    this.$messagebox.confirm('将重置为最新年报数据，所有修改的数据将丢失，你确定要继续吗？', '').then((res) => {
                        ...
                        //console.log("confirmconfirm", err);
                    })
                } else {
                }
            },
