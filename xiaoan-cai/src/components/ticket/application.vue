    ...
    export default {
        ...
        data() {
            return {
                ...
                title: "开票须知：",
                msg: [
                    "1、支付金额满￥299，方可开具发票。默认开具增值税普通发票，类型为「咨询服务费」。每月 20-25 日集中开具发票。",
                    "2、请务必仔细核对信息，发票一经开具，非我司原因将无法重开。",
                ],
                taiTouName: "发票抬头",
                taiTou: "",
        ...
        methods: {
            ...
            goApplicationImg() {
                this.$router.push({
                    path: '/applicationImg',
                    query: {weixinId: this.$route.query.weixinId, MathTime: global.timestamp}
                });
            },
            //添加发票
            addInvoice() {
                ///
                /*console.log("<==", this.$route.query.weixinId, "==", this.taiTou, "==", this.taiTou, "==", this.jinE, "==", this.pingzhengUrl, "==", this.person, "==", this.tel, this.province + this.selectcity + this.district + this.textarea, "==>");*/
                /*alert("<==", this.$route.query.weixinId, "==", this.taiTou, "==", this.taiTou, "==", this.jinE, "==", this.pingzhengUrl, "==", this.person, "==", this.tel, this.province + this.selectcity + this.district + this.textarea, "==>");*/
                this.$http({
                    method: "POST",
                    url: global.host + 'invoice/addInvoice',
                    params: {
                        /*invoiceId: '',//发票id*/
            ...
            fileUpdate() {
                let file = document.querySelector("#upfile").files[0];
                /*lrz(file).then(function (res) {
                  ...
                  .catch(function (err) {
                    // 处理失败会执行
                    console.log("filefilefilefile", err);
                  })
                  .always(function (adk) {
                    console.log("filefilefilefile", adk);
                    // 不管是成功失败，都会执行
                  });*/
                //let file = document.querySelector("#upfile").files[0];
                ...
                //console.log(param.post('file')); //FormData私有类对象，访问不到，可以通过get判断值是否传进去
                let config = {
                    headers: {'Content-Type': 'multipart/form-data'}
                };  //添加请求头
                this.matteSh = true;
                /**/
                this.$http.post(host + "invoice/upLoad", param, config).then(response => {
                    console.log(response.data);
                    this.fileStatus = true;
                    this.pingzhengText = "上传成功！";
                    this.matteSh = false;
                    $(".ticketFileTwo label span").css("color", "#7fc76d");
                    this.pingzhengUrl = response.data;
                    if (this.ticketListShuiHao == false) {
                        if (this.jinE.trim().length !== 0 && this.fileStatus == true) {
                            let jin = parseFloat(this.jinE);
                            if (jin >= 299) {
