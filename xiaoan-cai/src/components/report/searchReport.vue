<template>
    ...
    <div class="searchReport" id="searchReport">
        <div class="searchS">
            <div class="seaBox">
        ...
        <!--提示搜索-->
        <div class="prestrains" id="preStar" v-show="prestrains">
            <div v-for="(list,index) in prestrain" @click="gocontent(index)">
                <div class="prestrains_div" v-html="$array.key(values,list.subject)"></div>
            </div>
        </div>
    </div>
    </transition>
    ...
    methods: {
      ...
      searchList() {
        ...
        if (value == "" || value.length == 0) {
          ...
          setTimeout(() => {
            this.$router.push({path: "/listReport", query: {value: value, MathTime: global.timestamp}});
          })
        }
      },
      //搜索.get("")
...
<style scoped>
    ...
    #searchReport .prestrains div:active {
        /*background-color: #f0f1f5;*/
        opacity: 0.6;
    }
    #searchReport .prestrains_div {
