<template>
  <div class="calendarBox" style="overflow: hidden">
    <div class="date">
      <span @click="chooseTime" class="yearMonth left">{{now_year}}年{{now_month}}月 <span
      ...
      <div class="calendar" @touchstart="moveStart" @touchend="moveEnd">
        <ul id="d2">
          <li :class="{black:item.isNowMonth,gray:!item.isNowMonth,red:item.checked,today:item.old.isToday}"
              v-for="(item,index) in list" @click="checkMe(item.id-1)">
...
<script>
  // import $ from 'jquery'
  import {DatetimeView, Popup} from 'vux'
