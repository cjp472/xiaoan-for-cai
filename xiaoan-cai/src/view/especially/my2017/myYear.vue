<template>
    <div class="myYear">
        <audio id="myMusic" src="/static/audio/Free-Converter.com-20171225012043-1843765196.mp3" preload
               autoplay loop></audio>
        ...
        <div class="xiaoan-audio" @click="removeAudio">
            <img class="" :class="{circle: !circlePlay}" src="/static/img/myYear/yinyue@3x.png"/>
        </div>
            ...
            <swiper :options="swiperOption" ref="mySwiper">
                ...
                <swiper-slide>
                    <xiaoan-myYearEight :android="android" :zbyz="zbyz" :myYearData="myYearData" :active="swIndex"
                                        :myHome="myHome"
                                        @click="add"
                                        v-on:share-click="addshare"></xiaoan-myYearEight>
                </swiper-slide>
    ...
    export default {
        ...
        data() {
            return {
                ...
                myHome: {
                    ...
                    codeName: '信公咨询',
                    code: '',
                    Avatar: '/static/img/personal/timg@2x.png',
                },
                user_Id: '',
                code: '',
                shareDom: false,
        ...
        created() {
            ...
            if (this.user_Id == '' || this.user_Id.length == 0) {
                this.$nextTick(() => {
                    this.$storage.loginStatus(this, window.document.title, window.location.href, 'number');
                })
            }
        ...
        mounted() {
            ...
            let shareLink = decodeURI(global.currentIp + '#/myYear?code=' +
                this.myHome.code + '&name=' + this.myHome.name + '&Avatar=' + this.myHome.Avatar); // 分享链接
            let shareDesc = "见证喧闹纷繁的2017年股市，我欠自己一份图文总结。";//描述
            this.$weixin.wxshare(adverttitle, shareDesc, shareLink, global.shareImg3, true);
            // 处理安卓机太小屏幕问题
        ...
        methods: {
            ...
            userHome() {
                this.$http({
                    ...
                    if (res.data.returnCode == 1) {
                        ...
                        this.$nextTick(() => {
                           // this.$storage.loginStatus(this, window.document.title, window.location.href);
                        })
                    }
            ...
            getYearSummaryByCode(code) {
                this.$http({
                    method: 'POST',
                    url: host + "common/getYearSummaryByCode",
                    params: {
                        code: code
                    }
                }).then((res) => {
                    ...
                    if (res.data.returnCode == 1) {
                        if (res.data.returnObject == null || res.data.returnObject == '') {
                            ...
                            /**************************************************/
                            let adverttitle = "数据告诉你，2017年我在董办工作有多拼！";
                            let shareLink = global.currentIp + '#/myYear?code=' +
                                this.myHome.code + '&name=' + this.myHome.name + '&Avatar=' + this.myHome.Avatar; // 分享链接
                            let shareDesc = "见证喧闹纷繁的2017年股市，我欠自己一份图文总结。";//描述
                            this.$weixin.wxshare(adverttitle, shareDesc, shareLink, global.shareImg3, true);
                        }
            ...
            getCompanyCount() {
                this.$http({
                    method: 'GET',
                    url: host + "common/getCompanyCount",
                    params: {}
                }).then((res) => {
...
<style lang="scss">
    ...
    .myYear {
        ...
        .xa-pop {
            ...
            border-radius: 12px;
            .xa-pop-icon {
                background: url("/static/img/myYear/1-18@3x.png");
                width: 100%;
                height: 85px;
                background-repeat: no-repeat;
