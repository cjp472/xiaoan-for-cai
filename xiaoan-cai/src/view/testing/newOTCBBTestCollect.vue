            ...
            <footer class="newOTCBBTestFooter" ref="newOTCTFoot">
                ...
                <button @click="questionCollect">
                    <i class="icon-middleCollect">
                        <img src="/static/img/testing/testWrongCollectClose@3x.png" alt=""/>
                    </i>
                    <p v-show="false">收藏</p>
                    <p v-show="true">移除</p>
                </button>
...
<script>
    ...
    export default {
        ...
        mounted: function () {
            ...
            var sharelink2 = decodeURI(window.location.href.split("#/")[0] + "#/testing?MathTime=" + global.timestamp);//分享链接
            var shareDesc = "";//描述
            /**/
        ...
        methods: {
            ...
            getErrorQuestion(questionPage) {
                this.$http({
                    method: "POST",
                    url: this.$api.host + "sectest/selectCollect",
                    params: {
                        page: questionPage,
                    }
                }).then((res) => {
                    if (res.data.returnCode == 1) {
                        ...
                        this.$router.push({path: "/newOTCBBTestingEnd", query: {MathTime: global.timestamp}});
                    }
                    /*this.$nextTick(() => {
                        let newOTCTTT = this.$refs.newOTCTTT.getBoundingClientRect().height;
                        let newOTCT = this.$refs.newOTCT.getBoundingClientRect().height;
            ...
            questionBefore() {
                ...
                if (this.page < 1) {
                    this.page = 1;
                    this.$toast({message: '该题已经是第一题了，无上一题', position: 'middle', duration: 1000});
                } else {
                    this.getErrorQuestion(this.page);
            ...
            questionNext() {
                ...
                if (this.page > this.total) {
                    ...
                    this.$toast({message: '该题已经是最后一题了，无下一题', position: 'middle', duration: 1000});
                } else {
                    this.getErrorQuestion(this.page);
            ...
            remQuestion(questionID) {
                this.$http({
                    method: "POST",
                    url: this.$api.host + "sectest/deleteCollectQuestion",
                    params: {
                    ...
                    if (res.data.returnCode == 1) {
                        ...
                        this.$toast({message: '移除收藏成功', position: 'middle', duration: 1000});
                    } else {
                        this.$toast({message: '不好了，遇见未知错误', position: 'middle', duration: 1000});
                    }
                }).catch((err) => {console.error(err);
                    this.$toast({message: '移除收藏失败，请稍后尝试', position: 'middle', duration: 1000});
                })
            },
            lineHeight() {
...
<style scoped>
    #newOTCBBTestWrongList .icon-middleClose {
        height: 20px;
        width: 20px;
        background-image: url("/static/img/testing/testWrongclose@3x.png");
        background-repeat: no-repeat;
