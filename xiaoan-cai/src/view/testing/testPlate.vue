    ...
    methods: {
      ...
      startQa() {
        ...
        if (this.isSelected.id == undefined) {
          this.$toast({message: '请选择板块类别继续答题！', position: 'middle', duration: 1000});
        } else {
          this.$router.replace({path: "/testingList", query: {market: this.isSelected.id, MathTime: global.timestamp}});
