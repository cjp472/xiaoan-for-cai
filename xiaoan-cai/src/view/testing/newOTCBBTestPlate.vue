<template>
    <transition name="bounce">
        <div id="testPlate" class="newTestPlate">
            <div class="testingEnd-content-middle-item">
                <span class="testingEnd-content-middle1"><div></div></span>
                <span class="testingEnd-content-middle-text">{{text[0]}}</span>
                <span class="testingEnd-content-middle2"><div></div></span>
            </div>
            <div class="testPlate-cont">
                <div class="testPlate-content" v-for="(list,index) in marketItem" @click="checkedImg(index)">
                    <div class="testPlate-cont-icon">
                        <div :class="{iconImg:list.isSelected}"></div>
                    </div>
                    <div class="testPlate-text" v-html="list.name"></div>
                    <div class="testPlate-Doc">{{list.doc}}</div>
                    <div class="testPlate-cont-iconImg1"></div>
                </div>
            </div>
            <!--按钮答题-->
            <div class="testPlate-start" @click="startQa()">
                <button class="leftIcon">{{text[1]}}</button>
            </div>
        </div>
    </transition>
</template>

<script>
    export default {
        name: 'newTestPlate',
        data() {
            return {
                Sco: '',
                user_id: '',
                text: ['请选择考试题数', '开始答题'],
                marketItem: [
                    {id: 0, name: '25&ensp;&ensp;题', doc: '(建议时长15分钟)', number: 25, isSelected: false},
                    {id: 1, name: "50&ensp;&ensp;题", doc: '(建议时长30分钟)', number: 50, isSelected: false},
                    {id: 2, name: "100&ensp;题", doc: '(建议时长60分钟)', number: 100, isSelected: false},
                    {id: 3, name: "全真模拟", doc: '(建议时长120分钟)', number: 200, isSelected: false},
                ],
                isSelected: {},
            };
        },
        created() {
            var userId = this.$cookie.getCookie("userId");
            this.user_id = userId;
            console.log(this.user_id);
        },
        mounted() {
            document.body.addEventListener('touchstart', function () {
            });
        },
        methods: {
            checkedImg(index) {
                for (let i in this.marketItem) {
                    this.marketItem[i].isSelected = false;
                }
                this.marketItem[index].isSelected = true;

                this.isSelected = this.marketItem[index];
                console.log(this.isSelected);
            },
            startQa() {
                console.log(this.isSelected.id);
                if (this.isSelected.id == undefined) {
                    this.$toast({message: '请选择答题数', position: 'middle', duration: 1000});
                } else {
                    this.$router.replace({
                        path: "/newOTCBBTestingList",
                        query: {entrance: 1, total: this.isSelected.number, MathTime: global.timestamp}
                    });
                }
            }
        }
    }
</script>

<style lang="scss">
    .newTestPlate {
        width: 100%;
        height: 100%;
        overflow: hidden;
        background-color: #5B5C64;
        background-image: url("/static/img/testing/back2@3x.png");
        background-size: 100% 100%;
        .testingEnd-content-middle-item {
            text-align: center;
            color: #f0cf8d;
            padding: 10% 0 0 0;
            .testingEnd-content-middle1 {
                width: 60px;
                height: 1px;
                display: inline-block;
                border-bottom: 0.5px solid #f0cf8d;
                position: relative;
                top: -6px;
                div {
                    height: 3px;
                    width: 3px;
                    background-color: #f0cf8d;
                    position: absolute;
                    border-radius: 50%;
                    right: 0;
                    top: 0;
                }
            }
            .testingEnd-content-middle-text {
                padding: 0 12px;
                font-size: 23px;
            }
            .testingEnd-content-middle2 {
                width: 60px;
                height: 1px;
                display: inline-block;
                border-bottom: 0.5px solid #f0cf8d;
                position: relative;
                top: -6px;
                div {
                    height: 3px;
                    width: 3px;
                    background-color: #f0cf8d;
                    position: absolute;
                    border-radius: 50%;
                    left: 0;
                    top: 0;
                }
            }
        }
        .testPlate-cont {
            padding: 15.55% 0 0 18%;
            box-sizing: border-box;
            .testPlate-content {
                display: table;
                margin-bottom: 40px;
                .testPlate-cont-icon {
                    display: table-cell;
                    vertical-align: middle;
                    padding-right: 20px;
                    div {
                        width: 20px;
                        height: 20px;
                        background-color: rgba(240, 207, 141, 0.5);
                        border-radius: 50%;
                    }
                }
                .iconImg {
                    height: 20px;
                    width: 20px;
                    background-image: url("data:image/png;base64,");
                    background-repeat: no-repeat;
                    background-size: 100% 100%;
                }

                .testPlate-text {
                    display: table-cell;
                    font-size: 19px;
                    color: white;
                    line-height: 1;
                    vertical-align: bottom;
                }
                .testPlate-Doc {
                    display: table-cell;
                    font-size: 14px;
                    font-family: PingFangSC-Light, sans-serif, Microsoft Yahei, Helvetica;
                    color: #f0cf8d;
                    vertical-align: middle;
                }
                .testPlate-cont-iconImg1 {

                }
            }
        }
        .testPlate-start {
            text-align: center;
            position: absolute;
            bottom: 67.5px;
            left: 50%;
            margin-left: -97.5px;
            .leftIcon {
                width: 195px;
                height: 40px;
                border: none;
                background-color: #f0cf8d;
                color: white;
                font-size: 19px;
                text-align: center;
                line-height: 40px;
                padding: 0;
                font-family: PingFangSC-Regular, sans-serif, Microsoft Yahei, Helvetica;
                border-radius: 30px;
                outline: none;
            }
            .leftIcon:active {
                background-color: #e4c077;
            }
        }
    }
</style>
