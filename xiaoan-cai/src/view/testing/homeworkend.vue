...
<script>
    ...
    export default {
        ...
        data() {
            return {
                title: "homeworkend",
                successicon: "/static/img/testing/chenggong.png",
                text: "您已完成作业！",
                truenum: '4/5',
                usetime: "1'30s",
                listarr: [],
        ...
        methods: {
                ...
                this.$http({
                    ...
                    if (res.data.returnCode == 1) {
                        if (index !== undefined) {
                            this.$router.push({
                                path: "/homework",
                                query: {courseId: this.courseid, MathTime: global.timestamp, err: true, no: index}
                            });
                        } else {
                            this.$router.push({
                                path: "/homework",
                                query: {courseId: this.courseid, MathTime: global.timestamp, err: true}
                            });
                        }
                    } else if (res.data.returnCode == -3) {
                        //this.$toast({message: '没有错题', position: 'middle', duration: 1500});
                        if (show !== undefined) {
                            this.$router.push({
                                path: "/homework",
                                query: {courseId: this.courseid, MathTime: global.timestamp, err: true, no: index}
                            ...
                            this.$messagebox.confirm('您答对了所有作业，是否复习答题？', '').then(action => {
                                this.$router.push({
                                    path: "/homework",
                                    query: {courseId: this.courseid, MathTime: global.timestamp, err: true, no: 0}
                                });
                            })
                        }
            ...
            getduwu() {
                this.$http({
                    method: "GET",
                    url: this.$api.host + 'course/getStatute',
                    params: {
                        courseId: this.courseid
                    }
                }).then((res) => {
            ...
            getresult() {
                this.$http({
                    method: "GET",
                    url: this.$api.host + 'course/getAnswerResult',
                    params: {
                        courseId: this.courseid
                    }
                }).then((res) => {
