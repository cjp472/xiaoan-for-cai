        ...
        <footer class="testPlate-cont-submit">
            ...
            <button @click="testClose()" v-if="payMode == 1">
                <i class="icon-middleCollect">
                    <img src="/static/img/testing/testClose@3x.png" alt=""/>
                </i>
                <p v-show="true">退出</p>
            </button>
    ...
    export default {
        ...
        data() {
            return {
                ...
                XIAO: '../../static/img/testing/xiaodian.png',
                user_id: '',
                market: '',
                ScoType: '',//（单选还是选）
                primo: '1',//问题
        ...
        mounted() {
            ...
            let sharelink2 = decodeURI(window.location.href.split("#/")[0] + "#/testing?MathTime=" + global.timestamp);//分享链接
            let shareDesc = "";//描述
        ...
        methods: {
            exitTest() {
                this.$http({
                    method: "POST",
                    url: this.$api.host + "sectest/exitTest",
                    params: {
                        testId: this.TestId,
                    }
                }).then((res) => {
                    if (res.data.returnCode == 1) {
                        this.$router.replace({path: '/newOTCBBTestingEnd', query: {MathTime: global.timestamp}});
                    } else {
                        this.$router.replace({path: '/newOTCBBTestingEnd', query: {MathTime: global.timestamp}});
                    }
                }).catch((err) => {console.error(err);
                    this.$toast({message: '遇到未知错误，无法退出，将返回主页', position: 'middle', duration: 3000});
            ...
            startQa() {
                ...
                this.$http({
                    method: "POST",
                    url: this.$api.host + "sectest/xsbStart",
                    params: {
                        ...
                        setTimeout(() => {
                            this.$nextTick(() => {
                                this.$router.replace({
                                    ...
                                    query: {MathTime: global.timestamp}
                                });
                    ...
                    this.$toast({message: '', position: 'middle', duration: 1000});
                    this.$router.replace({path: "/newOTCBBTestingEnd", query: {MathTime: global.timestamp}});
                })
            },
            /*安卓机兼容*/
            ...
            commit() {
                ...
                this.$http({
                    method: "POST",
                    url: this.$api.host + "sectest/xsbCommit",
                    params: {
                        testId: this.TestId,
            ...
            toNext() {
                ...
                this.$http({
                    method: "POST",
                    url: this.$api.host + "sectest/getNextQuestion",
                    params: {
                        testId: this.TestId,
                        questionId: this.QaIdS,
                        choseIds: this.nextQa,
                    ...
                    if (res.data.returnCode == 1) {
                        ...
                        this.$toast({message: res.data.returnMsg, position: 'middle', duration: 2000});
                    }
                    this.submit = true;
                }).catch((err) => {console.error(err);
                })
            ...
            beforeSubmit() {
                ...
                this.$http({
                    method: "POST",
                    url: this.$api.host + "sectest/getLastQuestion",
                    params: {
                        testId: this.TestId,
                        questionNum: this.primo,
                    }
            ...
            testSubmit(index) {
                if (this.submit == true) {
                    if (index == 0) {
                        ...
                        // 返回上一题
                        if (this.primo == 1) {
                            this.$toast({message: '没有上一题了', position: 'middle', duration: 2000});
                        } else {
                            this.beforeSubmit();
                        }
                    } else if (index == 1) {
                        ...
                        if (this.primo < this.marketAll) {
                            ...
                            if (this.ScoType == 3) {
                                ...
                                if (this.tiarr.length > 1) {
                                    ...
                                    this.$toast({message: '请至少勾选两项', position: 'middle', duration: 2000});
                                }
                            } else {
                                //danxuanhuoz panduan
                                if (this.tiarr == null || this.tiarr.length === 0) {
                                    this.$toast({message: '请勾选选项', position: 'middle', duration: 2000});
                                } else {
                                    this.submit = false;
                                    this.toNext();
                            ...
                            this.$toast({message: '出现未知错误', position: 'middle', duration: 2000});
                        }
                    } else {
                        console.log('NAN');
                    }
                } else {
                    this.$toast({message: '网络不稳定', position: 'middle', duration: 2000});
                }
            },
        }
    }
    ...
    .testingListOTCBB {
        ...
        .testingList-trends {
            ...
            .testingList-item {
                ...
                position: relative;
                .testingList-itemG {
                    background: url("/static/img/testing/xiaodian.png");
                        ...
                        .testPlate-units-TitleS {
                            ...
                            .testPlate-units-Title {
                                ...
                                .unitsIcon {
                                    ...
                                    height: 20px;
                                    width: 20px;
                                    background-image: url("/static/img/testing/clickd@3x.png");
                                    background-repeat: no-repeat;
                                    background-size: 100% 100%;
                                }
                            }
        ...
        .testPlate-toButton {
            background-image: url("/static/img/testing/toButtom.png");
            background-size: 100% 100%;
            background-repeat: no-repeat;
            height: 20px;
            width: 32.76px;
