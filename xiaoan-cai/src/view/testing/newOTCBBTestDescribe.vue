    ...
    export default {
        ...
        methods: {
            ...
            payMoney() {
                ...
                this.$http({
                    method: 'POST',
                    url: this.$api.host + "checkWxIdByUserId",
                    params: {}
                }).then((res) => {
                    let wxID = res.data.returnObject;
                    if (wxID === null || wxID == "") {
                        this.$toast({message: '请重新登录付费', position: 'middle', duration: 3000});
                        this.exit();
                    } else {
            ...
            WxJSBridge(vue, totalFee, wxId) {
                ...
                vue.$http({
                    method: 'POST',
                    url: this.$api.host + "wxpay/payexam",
                    params: {
                        totalFee: totalFee,
                        openId: wxId,
                    }
                        ...
                        WeixinJSBridge.invoke(
                            ...
                            function (res) {
                                if (res.err_msg == "get_brand_wcpay_request:ok") {
                                    ...
                                    vue.$toast({message: '支付成功', position: 'middle', duration: 3000});
                                    vue.$router.push({
                                        path: "/newOTCBBTestingEnd",
                                        query: {MathTime: global.timestamp}
                                    });
                                } else if (res.err_msg == "get_brand_wcpay_request:cancel") {
                                    vue.$toast({message: '取消支付', position: 'middle', duration: 3000});
                                } else {
                                    vue.$toast({message: '支付失败', position: 'middle', duration: 3000});
                                }
                            }
                        );
                    }
