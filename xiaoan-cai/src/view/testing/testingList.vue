  ...
  export default {
    ...
    methods: {
      ...
      startQa() {
        this.$http({
          method: "POST",
          url: this.$api.host + "sectest/start",
          params: {
            market: this.market,
          }
        }).then((res) => {
          if (res.data.returnCode == 1) {
            ...
            this.$router.replace({path: "/testingEnd", query: {MathTime: global.timestamp}});
          }
        }).catch((err) => {
          console.error(err);
        })
      ...
      commit() {
        ...
        this.$http({
          method: "POST",
          url: this.$api.host + "sectest/commit",
          params: {
            testId: this.TestId,
          ...
          this.$router.replace({path: "/testLoading", query: {MathTime: global.timestamp}});
          /*MtaH5.clickShare('dongminenglicep-2', {testsubmit: true});*/
      ...
      toNext() {
        ...
        this.$http({
          method: "POST",
          url: this.$api.host + "sectest/toNext",
          params: {
            testId: this.TestId,
            questionId: this.QaIdS,
            choseIds: this.nextQa,
      ...
      testSubmit() {
        if (this.isNexts == "1") {
          ...
          if ($('.unitsIcon div').html() == undefined) {
            //this.toNext();
            this.$toast({message: '请勾选选项', position: 'middle', duration: 1000});
          } else {
            this.toNext();
...
<style scoped>
    ...
    #testingList .testingList-text {
        ...
        right: -22px;
        top: 6px;
        background-image: url("/static/img/testing/dijiti.png");
