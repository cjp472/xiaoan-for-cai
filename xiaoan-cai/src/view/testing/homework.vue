<template>
  <div id="homework">
    <div class="content">
      ...
      <div class="select">
        <ul>
          <li v-for="(list,index) in liarr" @click="selectitem(index)">
    ...
    <div class="footer">
      <div class="up" v-for="(list,index) in footerarr" @click="fclick(index)">
...
<script>
  export default {
    ...
    data() {
      return {
        footerarr: [
          {name: "上一题", icon: "/static/img/testing/uparrow.png"},
          {name: "解析", icon: "/static/img/testing/jiexi.png"},
          {name: "下一题", icon: "/static/img/testing/downarrow.png"}
        ],
        liarr: [],
        jiexishow: false,
        itemtime: 0, // 每道题的时间
    ...
    created() {
      ...
      if (this.$route.query.courseId !== undefined) {
        ...
        this.$toast({message: '链接错误，三秒后跳出', position: 'middle', duration: 1500});
        setTimeout(() => {
    ...
    methods: {
      ...
      getresultinfo(index) {
        this.$http({
          method: "GET",
          url: this.$api.host + "course/getQuestions",
          params: {
          ...
          if (res.data.returnCode == 1) {
            ...
            console.log(res);
            this.$weixin.wxAuthRedirect('COURSE-' + this.courseid);
          }
        }).catch((err) => {
          console.error(err)
      ...
      geterrinfo(index) {
        this.$http({
          method: "GET",
          url: this.$api.host + "course/getErrorResult",
          params: {
            courseId: this.courseid
          }
        }).then((res) => {
      ...
      xxx() {
        ...
        if (j == 0) {
          this.$router.replace({
            path: "/homeworkend",
            query: {courseId: this.courseid, MathTime: global.timestamp}
          });
        }
      },
      //单选操作
      selectitem(index) {
        ...
        if (this.jiexishow !== true) {
          ...
          /*this.$toast({message: '开启下一题吧', position: 'middle', duration: 1500});*/
        }
        //console.log($("li"))
      },
      contentHeight() {
          ...
          if (this.errstatus == true) {
            if (this.no <= 0) {
              ...
              if (this.isallitem == true) {
                this.$toast({message: '没有上一题', position: 'middle', duration: 1500});
              } else {
                this.$toast({message: '没有错题了', position: 'middle', duration: 1500});
              }
            ...
            //点击作业上一题
            /*console.log(index, '上一题', this.no, 'this.userselarr[this.no]', this.allarr, this.userselarr[this.no])
            this.jiexishow = false;
            if (this.no <= 0) {
                this.$toast({message: '没有上一题了', position: 'middle', duration: 1500});
            } else {
                this.no -= 1;
                this.getinfo(this.no)
                console.log('sdafasdfasdfdsf', this.userselarr)
                for (var i in $('li')) {
                    $('li').eq(i).find('span').removeClass('selecti');
                }

                if (this.userselarr[this.no] == undefined) {
                    $('li').eq(this.userselarr[this.userselarr.length - 1]).find('span:first-child').addClass('selecti');

                } else if (this.userselarr[this.no] == '') {
                    for (var i in $('li')) {
                        $('li').eq(i).find('span').removeClass('selecti');
                    }
                    console.log('this.no', this.no, 'this.allarr', this.allarr)
                    if (this.allarr[this.no].chosenOptionId == null || this.allarr[this.no].chosenOptionId == '') {
                        this.jiexishow = true;
                    }
                } else {
                    this.userselarr[this.no] == '' ? this.userselarr[this.no - 1] : $('li').eq(this.userselarr[this.no]).find('span:first-child').addClass('selecti');
                }
            }*/
          }
        } else if (index == '1') {  //解析
          ...
          if (this.errstatus == true) {
            ...
            if (this.no >= this.nolength - 1) {
              ...
              if (this.isallitem == true) {
                this.$toast({
                  message: '没有更多题目,\n' +
                  '点击“返回”上一页', position: 'middle', duration: 1500
                });
              } else {
                this.$toast({
                  message: '没有更多错题,\n' +
            ...
            if ($(".selecti").length !== 0 || this.jiexishow == true) {
              ...
              if (this.no >= 5) {
                ...
                if (this.userselect == '') {
                  ...
                  /*if (this.allarr[this.no].isAnswer == true && this.allarr[this.no].chosenOptionId == null) {
                      this.jiexishow = true;
                  }*/

                  /*console.log('this.allarr+++++++', this.no, this.allarr, this.jiexishow)*/
                  /*for (var i in $('li')) {
                      $('li').eq(i).find('span').removeClass('selecti');
                  }
                  if (this.userselarr.length > 4) {
                      this.userselarr = [];
                      this.userselarr.push('');
                  } else {
                      this.userselarr.push('');
                  }*/
                  ...
                  this.$http({
                    method: "GET",
                    url: this.$api.host + 'course/answerRecord',
                    params: {
                  ...
                  /*if (this.userselarr.length > 4) {
                      this.userselarr = [];
                      this.userselarr.push(this.userselarrstatus);
                  } else {
                      this.userselarr.push(this.userselarrstatus);
                  }*/
                  //提交答题记录的时候 处理时间
                      ...
                      if (this.no >= 5) {
                        this.$router.replace({
                          ...
                          query: {courseId: this.courseid, MathTime: global.timestamp}
                                      ...
                                      if(this.no>=5){
                                          this.$router.push({path: "/homeworkend", query: {courseId:this.courseid,MathTime: global.timestamp}});
                                      }
                                  }*/ else {
                      this.$toast({
                        message: res.data.returnMsg,
              ...
              this.$toast({message: '请勾选项', position: 'middle', duration: 1500});
            }
...
<style scoped>
  ...
  #homework li .selecti {
    background-image: url("/static/img/testing/SELECTED.png");
