...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //
            jump(i) {
                var y = parseInt(this.offsetArr[i]);
            ...
            //
            muluShow() {       //-----目录相关方法-----
                if (this.muluCode == true) {
                    setTimeout(() => {
                        var hei = document.body.clientHeight;
                        var scroll = (hei - 45) + 'px';
            ...
            getDetails() {
                this.$http({
                    ...
                    if (res.data.returnCode === 1) {
                        ...
                        //
                        this.$nextTick(() => {
                            console.log('12212121212', this.$refs.contTextDom.offsetTop);
                            console.log('12212121212', this.$refs.rulesCountDom.clientHeight);
                            console.log('12212121212', this.$refs.footerDom.clientHeight);
            ...
            contentDom() {
                this.$nextTick(() => {
                    ...
                    if (strLen.length == 0) {
                        ...
                        // for (let i in strLen) {
                        //     if (strLen[i].innerHTML.indexOf("&nbsp;") > -1) {
                        //         let sarr = strLen[i].innerHTML.split("&nbsp;");
                        //         strLen[i].innerHTML = this.ImpArr(sarr);
                        //         console.log("32123231", this.ImpArr(sarr));
                        //     }
                        // }
                    }
                })
            ...
            rees() {
                //目录获取
                for (var i in $('#rules-contents h2 span')) {
                    if ($("span").eq(i).attr('lang') == 'EN-US') {
                        if ($("span").eq(i).text().length >= 20) {
                            var xx = $("span").eq(i).text().substring(0, 21); //截取到22个字符 但是6.2版本竟然重字 还是被挤下来 =》21
                            $("span").eq(i).text(xx);
                        }
                    }
                }
            ...
            scrollToElement(container, child) {
                container = $(container);
                var search = $(".searchs").outerHeight();
                var countTil = $(".countTli").outerHeight();
                console.log(search + countTil);
                var tobeHeight = search + countTil;
                child = $(child);
                console.log("234324suoyi", $(child));
                if (child.length == 0) {
                    console.log(0);
                } else {
                    var conScrollTop = container.scrollTop();
                    console.log(conScrollTop);
                    var childOffsetTop = child.position().top;
