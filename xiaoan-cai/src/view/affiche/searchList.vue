<template>
  <div class="ruleslist" id="affiche">
    <div class="searchs">
    ...
    <div class="listGroup" id="listGroup">
      <ul class="infinite-list" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading"
        ...
        <li class="anList" v-for="(list,index) in ruleslist" @click="goCount(index)">
          <div style="width: 100%">
            <span class="content-text" v-html="key(list.title)"></span>
            <p style="float: right">{{format(list.releaseDate)}}</p>
          </div>
        </li>
        <li class="anList seniorText">
                    <span class="trytext" @click="showSenior"
                          style=" text-decoration: none;border-bottom: 0.5px solid;">搜索不到想要的内容，试试看全文检索吧!</span>
      ...
      <div class="noInfo" v-show="noInfoShow500">
        <div class="trySenior" :class="{show:noInfoShow500}" style="width:100%">
          <img src="/static/img/warning/warnin.png" alt="网络连接失败">
          <div class="tryText" style="display: flex;justify-content: center;width:100%;">
...
<script>
  ...
  export default {
    ...
    watch: {
      xianshi: function (val, old) {
        if (old == false) {
          ...
          for (let i = 0; i < this.value0.length; i++) {       // 然后 板块类别条件选中状态 只有确认提交过的条件才能显示
      ...
      xianshi1: function (val, old) {
        if (old == false) {     // 公告类别筛选条件栏展开
          ...
          for (let i = 0; i < this.value1.length; i++) {       // 然后 公告类别条件选中状态 只有确认提交过的条件才能显示
      ...
      xianshi2: function (val, old) {
        if (old == false) {
          ...
          let arr = [];
          for (let i in this.hangyeStatus) {
            for (let j in this.hangyeStr) {
              if (this.hangyeStr[j] == this.hangyeStatus[i]) {
                arr.push(this.hangyeStatus[i])
              }
            }
          }
          for (let i in $(".showSearch2").find('.aaa')) {       // 关闭当前筛选条件栏后 首先重置所有的选中样式
          ...
          for (let i = 0; i < this.value2.length; i++) { // 然后 行业类别条件选中状态 只有确认提交过的条件才能显示
    ...
    updated() {
      ...
      let SXpup = (hei - 90) + "px";
      let hgt = (hei - (document.querySelector('.searchs').clientHeight) - (document.querySelector('.xa-Footer').clientHeight) - (document.querySelector('.searchBox').clientHeight)) + 'px';
    ...
    mounted() {
      ...
      if (window.localStorage.getItem('dateStart') !== null) {
        let str = window.localStorage.getItem('dateStart');
        this.timesDate1 = str;
        this.times1();
      ...
      if (window.localStorage.getItem('dateEnd') !== null) {
        let str = window.localStorage.getItem('dateEnd');
        this.timesDate2 = str;
        this.times2();
    ...
    methods: {
      ...
      value2text() {
        ...
        if (this.value2.length == 1 && this.value2[0].length == 0) {
          ...
          for (let i = 0; i < this.value2.length; i++) {       //然后 板块类别条件选中状态 只有确认提交过的条件才能显示
            $(".search3").find('span').text(this.value2[0].substring(0, 3) + '...')
            $(".tip2").text(this.value2.join(',').substring(0, 8) + '...')
          }
        }
...
//        }
      ...
      hideGuize() {
        ...
        for (let i = 0; i < this.guizeIdStatus.length; i++) {
          for (let j = 0; j < this.hangyeStatus.length; j++) {
            guizeStr = this.hangyeStatus[j];
            if (guizeStr.split(',').length > 1) {     //如果一个id值里面有好几个id
              guizeStr.split(',')
              for (let x in guizeStr.split(',')) {
      ...
      hideTime() {
        ...
        for (let i in timeText) {
          for (let j in arr) {   //timeText.eq(i).parent().find('.itemTip').removeClass('selected').attr('select', 'true');
      ...
      SXhide() {
        ...
        let guizeText = $('.sxitemBox:nth-child(2)').find(".listText");
        let marketText = $('.sxitemBox:nth-child(1)').find(".listText");
        if (this.markerStr.length == 1 && this.markerStr[0] == '') {
          this.value0 = [];
          $(".search1").find('span').text("板块类别")
          for (let i = 0; i < marketText.length; i++) {        //多余的数组元素 去掉选中状态
            marketText.eq(i).parent().find('.itemTip').removeClass('selected').attr('select', 'true');
          }
        }
      ...
      SXshow() {
        ...
        let unitsText = $('.sxitemBox:nth-child(3)').find(".listText");
        let guizeText = $('.sxitemBox:nth-child(2)').find(".listText");
        let marketText = $('.sxitemBox:nth-child(1)').find(".listText");
        ...
        this.$nextTick(() => {
          ...
          let trueArr2 = [];            //侧边栏 行业类别属性select=false选中的数组
          ...
          for (let i = 0; i < this.value2.length; i++) {      //行业类别同步状态
        ...
        this.$nextTick(() => {
          ...
          for (let i = 0; i < this.value1.length; i++) {       //公告类别同步状态
          ...
          let trueArr = [];             //侧边栏 公告类别属性select=false选中的数组
        ...
        this.$nextTick(() => {
          ...
          for (let i = 0; i < this.value0.length; i++) {       //公告类别同步状态
      ...
      SortSH(event) {
        ...
        if (this.value0.length == 0) {
          ...
          /*console.log('选中状态开启!')*/
          for (let i = 0; i < this.value2.length; i++) { //首先循环选中状态的行业类别值
            $(".search3").find('span').text(this.value2[0].substring(0, 3) + '...');
            $(".tip2").text(this.value2.join(',').substring(0, 8) + '...')
            for (let q = 0; q < this.sxlistItem[0].item.length; q++) {  //循环板块类别数组
              for (let n = 0; n < this.value0.length; n++) {      //循环是否选中的板块类别
                if (this.value0[n] == this.sxlistItem[0].item[q].name) {    //若选中板块类别则返回其在guizeArr中的下标值
                  for (let j = 0; j < this.guizeArr[q].length; j++) { //然后循环选中的板块类别下的行业类别数组
      ...
      redCircleStatus() {
        ...
        for (let i = 0; i < stausArr.length; i++) {
      ...
      yeah() {        //点击确定发送请求数据
        ...
        /*  $(".ceshadow").show();//加载中出现*/
        let unitIds = this.idarr1.join(',');
        // 行业类别id值 如果只选了板块类别 cateIds就填板块类别的id  如果板块类别和行业类别都填了 cateIds只写行业类别的id
        let cateIds = '';
        let timelinessIds = this.idarr3.join(',');
        let releaseStart = this.dateStart;
        let releaseEnd = this.dateEnd;
        ...
        // 板块类别数组处理字符串
        let marketString = this.value0.join(',');
        // 行业类别数组处理字符串
        let hangyeString = this.value2.join(',');
...
//          console.log('--------------->yeah', this.searchscope)
          ...
          if (res.data.returnCode == 1) {
            ...
            if (this.ruleslist.length > 0 && res.data.returnObject.list !== null) {
              for (let i in res.data.returnObject.list) {
      ...
      contentIncludesText() {
        if (this.contentIncludes.trim().length === 0) {
          ...
          if (this.contentIncludes.trim().indexOf(" ") !== -1) {
            ...
            let contIn = this.contentIncludes.split(" ");
            this.ImpArr(contIn);
          ...
          if (this.contentMaybeIncludes.trim().indexOf(" ") !== -1) {
            ...
            let contMaBe = this.contentMaybeIncludes.split(" ");
          ...
          if (this.contentExcludes.trim().indexOf(" ") !== -1) {
            ...
            let contEx = this.contentExcludes.split(" ");
            this.ImpArr(contEx);
...
//+++++++++++++++++++++++++++++++++++++++++//
        ...
        if (topBtn === '标题' && buttonBtn === '精确') {
          ...
          if (this.titleIncludes.trim().length === 0) {
            ...
            if (this.titleIncludes.trim().indexOf(" ") !== -1 || this.titleMaybeIncludes.trim().indexOf(" ") !== -1 || this.titleExcludes.trim().indexOf(" ") !== -1) {
              ...
              ////转换成数组后去除空元素在用逗号拼接
              let tilIn = this.titleIncludes.split(' ');
              this.ImpArr(tilIn);
              this.titleIncludes = tilIn.join(",");
              //console.log(this.titleIncludes);
              //转换成数组后去除空元素在用逗号拼接
              let tilMaBe = this.titleMaybeIncludes.split(" ");
              this.ImpArr(tilMaBe);
              this.titleMaybeIncludes = tilMaBe.join(",");
              //console.log(this.titleMaybeIncludes);
              //转换成数组后去除空元素在用逗号拼接
              let tilEx = this.titleExcludes.split(" ");
              this.ImpArr(tilEx);
      ...
      popupShow() {
        ...
        let couHeight = window.screen.height - 120;
        //alert(couHeight);
        let contop = couHeight / window.screen.height * 100;
        ...
        if (Array.isArray(this.titleIncludes)) {
          ...
          console.log("有逗号");
          let arr = this.titleIncludes.split(',').join(" ");
...
<style>
  #affiche ._v-container > ._v-content > .loading-layer {
...
<style scoped>
  ...
  .toLeft {
    /* width: 10px;
         height: 10px;*/
    display: block;
    /*background-image: url("/static/img/rules/Toleft.jpg");
        background-size: contain;
        background-repeat: no-repeat;*/
    font-size: 14px;
    position: absolute;
  ...
  .search-s .pass p {
    margin-bottom: 3px;
    background-image: url("/static/img/rules/selectImgUp.png");
  }
  .search-mrpx .imgBox {
    margin: 0;
    padding: 0;
    ...
    height: 4.5px;
    margin-left: 5px;
    background-image: url("/static/img/rules/selectImg.png");
    background-size: contain;
  }
  ...
  .closeDate { /*清除时期x按钮*/
    ...
    width: 15px;
    height: 15px;
    background-image: url("/static/img/rules/promptclose.png");
    background-repeat: no-repeat;
    background-size: contain;
  }
  .sxListsScroller {
