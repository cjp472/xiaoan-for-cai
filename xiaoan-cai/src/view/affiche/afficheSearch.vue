<template>
  <div class="affiche-search search" ref="afficheSearchDom">
    <div class="searchs">
      <div class="seabox">
        <div class="simpleBox">
    ...
    <div class="groupDsearch">
      ...
      <div class="dseResult dseMixHeight">
        <button type="button" v-for="(list,index) in Newarr" @click="dseResultOne(index)">{{list}}</button>
      ...
      <div class="dseResult">
        <button type="button" v-for="(list,index) in hotResult" @click="dseResultTwo(index)">{{list}}
        </button>
      </div>
      <div class="showBox">
    ...
    <div class="prestrain" id="preStar">
      <div v-for="(list,index) in prestrain" @click="gocontent(index)">{{list.title}}</div>
    </div>
    <!--提示公司代码/简称-->
    <div class="codeTip">
      <div class="codeTipText" v-for="(list,index) in codeTip" @click="companyContent(index)">
    ...
    methods: {
      ...
      searchList() {
        ...
        if (this.SIMvalues.length !== 0 && this.values.trim().length == 0) {
          ...
          this.$http({
            method: "GET",
            url: this.$api.host + 'common/autoComplete', // host: http://api.in-hope.cn/v1/
            params: {
              key: this.SIMvalues
            }
          }).then((res) => {
            ...
            console.log(err)
            this.$toast({message: '网络错误', position: 'middle', duration: 2000});
          })
      ...
      dseResultOne(index) {
        let str = this.defaultResult[index];
        let pattern = /^[0-9]\d*$/;
        let flag = pattern.test(str);
        if (flag === true || str.indexOf('*st') >= 0 || str.indexOf('*ST') >= 0 || str.indexOf('ST') >= 0 || str.indexOf('st') >= 0 || str.indexOf('SH') >= 0 || str.indexOf('sh') >= 0 || str.indexOf('SZ') >= 0 || str.indexOf('sz') >= 0) {
          // 公司代码/简称
...
<style scoped lang="scss">
  ...
  .search .seabox input::-webkit-input-placeholder {
    font-weight: lighter !important;
  }
  .search .icon-tripsousuo {
    font-size: 25px;
